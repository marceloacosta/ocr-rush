...
export async function checkGame(browser){
 ...
 const run=async()=>{await page.locator('#run').click();await page.waitForFunction(()=>!document.getElementById('run').disabled);};
 try{
  const response=await page.goto(origin);assert.ok(!(await response.text()).includes('__PUBLIC_URL__'));
  ...
  assert.equal(await page.locator('meta[property="og:image"]').getAttribute('content'),origin+'/social-preview.png');assert.equal((await page.request.get(origin+'/social-preview.png')).status(),200);
  ...
  await page.locator('#share-copy').click();assert.match(await page.evaluate(()=>navigator.clipboard.readText()),/AWS architecture game/);
  ...
  await page.keyboard.press('Escape');await page.reload();assert.match(await page.locator('#welcome-progress').innerText(),/1 of 6 levels completed/);assert.match(await page.locator('#start').innerText(),/Continue playing/);await page.locator('#start').click();assert.equal(await page.locator('[data-power="demand"]').getAttribute('aria-pressed'),'true');assert.equal(await page.locator('[data-contract="0"]').getAttribute('aria-label'),'Level 1: Receipt uploads, completed');
  await page.reload();await page.locator('#start').click();await page.locator('[data-contract="2"]').click();assert.match(await page.locator('#level-position').innerText(),/LEVEL 3 OF 6/);
  for(const width of [320,390,760,1024,1440]){await page.setViewportSize({width,height:900});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`game overflow ${width}`);await page.reload();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`welcome overflow ${width}`);assert.ok(await page.evaluate(()=>{const top=s=>document.querySelector(s).getBoundingClientRect().top;return top('.welcome h1')<top('#level-map')&&top('.newsletter-card')<top('#level-map');}),`welcome reading order ${width}`);if(width===390)await page.screenshot({path:'/private/tmp/ocr-game-welcome-mobile.png',fullPage:true});await page.locator('#start').click();}
  const target=simulate({...DEFAULT,power:'demand',layout:2},0);assert.ok(target.passed);await page.goto(challengeURL(origin,target));assert.match(await page.locator('#welcome-challenge').innerText(),/shared challenge for level 1/i);await page.locator('#start').click();await page.locator('#reset').click();await page.locator('[data-power="demand"]').click();await run();assert.match(await page.locator('.challenge-comparison').innerText(),/less than the shared design/);
  ...
  await page.goto(origin);await page.locator('#start').click();assert.match(await page.locator('#progress-summary').innerText(),/6 of 6/);await page.locator('#view-completion').click();assert.match(await page.locator('#share-caption').inputValue(),/completed all six levels/);
 ...
 try{
  await publicContext.addInitScript(()=>Object.defineProperty(navigator,'share',{value:async data=>{window.__shared=data;},configurable:true}));
