...
import {modelAssumptions,costDetails,latencyDetails} from './research.js?v=review-2';
import {showNewsletterInvite} from './newsletter.js?v=inline-signup-1';
const $=id=>document.getElementById(id),esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
let saved=null,canSave=true;try{saved=readProgress(localStorage.getItem(PROGRESS_KEY))||readProgress(localStorage.getItem('ocr-rush-progress-v4'));}catch{canSave=false;}
...
function renderProgress(){
 ...
 $('start').innerHTML=`${incomingChallenge.result?'Try this shared design':saved||count?'Continue playing':'Play the free game'} <span>→</span>`;
 ...
 const challenge=incomingChallenge.result;let notice=challenge?(challenge.passed?`<h2>A shared challenge for level ${incomingChallenge.stage+1}</h2><p>This design meets all the level’s requirements for <strong>${money(challenge.cost)} USD</strong>. Its configuration is loaded. Replay it, or change the design and try to complete the same workload for less.</p>`:`<h2>A shared attempt for level ${incomingChallenge.stage+1}</h2><p>This attempt delivered ${challenge.onTime.toLocaleString('en-US')} of ${challenge.target.toLocaleString('en-US')} valid PDFs on time for <strong>${money(challenge.cost)} USD</strong>, but did not meet all the requirements. Its configuration is loaded so you can replay it and try a change.</p>`):incomingChallenge.error?`<p>${esc(incomingChallenge.error)}</p>`:'';
 if(incomingChallenge.updated)notice+='<p>This design was shared before the failure rules were corrected. Its result and cost have been recalculated using the current rules.</p>';
...
function modal(title,content,kicker='BEHIND THE FACTORY'){modalTrigger=document.activeElement;$('dialog-body').innerHTML=`<p class="kicker">${kicker}</p><h2 id="dialog-title">${title}</h2>${content}`;$('dialog').showModal();}
...
function showResults(previous){
 ...
 $('share').disabled=false;$('share').textContent='Share this result';
...
function openShare(campaign=false){
 if(!campaign&&(!active||designKey(config)!==designKey(active.config)))return;
 openResultShare({result:campaign?(runs[stage].at(-1)||passed[stage]):active,best:passed,campaign,modal});
}
$('share').onclick=()=>openShare();
$('save').onclick=async()=>{try{await downloadResultCard({result:passed[stage],best:passed,campaign:true});toast('Completion image download started.');}catch{toast('The image could not be downloaded. Try sharing your completion instead.');}};
$('share-campaign').onclick=()=>openShare(true);$('view-completion').onclick=()=>openShare(true);
const shared=new URLSearchParams(location.search).get('contract');if(incomingChallenge.result){stage=incomingChallenge.stage;config={...incomingChallenge.result.config};}else if(CONTRACTS.some(c=>c.id===shared)){stage=CONTRACTS.findIndex(c=>c.id===shared);config=passed[stage]?.config||{...DEFAULT};}
